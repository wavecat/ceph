...
int RGWGetObj_Decompress::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
  ...
  while (first_block <= last_block) {
    ...
    if (ofs_in_bl + first_block->len > bl_len) {
...
void RGWGetObj_Decompress::fixup_range(off_t& ofs, off_t& end)
{
  if (partial_content) {
    ...
    if (cs_info->blocks.size() > 1) {
      ...
      auto cmp_u = [] (off_t ofs, const compression_block& e) { return ofs < e.old_ofs; };
      auto cmp_l = [] (const compression_block& e, off_t ofs) { return e.old_ofs < ofs; };
